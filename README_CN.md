...
### 3.3 CIFAR-10 视觉分类 (LRP 验证)

我们在 ResNet-18 上进行了长周期 (Long-Range Plan) 验证，证明了 ARS2-Neo 在视觉任务中的卓越性能。

| 优化器 | 配置 | Epochs | Best Acc | Final Acc | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Muon | Standard | 100 | 93.76% | 93.69% | 纯几何优化基线 |
| AdamW | Standard | 100 | 94.60% | 94.47% | 强力欧氏空间基线 |
| ARS2-Neo (Base) | k=0 | 100 | 95.58% | 95.52% | 能量-几何解耦的独立增益 |
| **ARS2-Neo (Sync)** | **ρ=0.1, k=1** | **60** | **95.87%** | **95.73%** | **SOTA 性能，收敛极快** |
| ARS2-Neo (Sync) | ρ=0.5, k=1 | 60 | 95.28% | 95.23% | 扰动半径过大导致精度损失 |
| **ARS2-Neo (AGA)** | **L=2.0** | **20** | 94.10% | 94.09% | **高效复用，近乎无损** |

**实验洞察**：

1. **超越 AdamW**: ARS2-Neo (Base) 仅凭能量-几何解耦就超越了调优后的 AdamW，证明了流形优化的本质优越性。
2. **平坦度红利**: 引入 ρ=0.1 的平坦度约束后，精度进一步提升至 95.87%，且仅需 60 Epoch 即可超越 AdamW 100 Epoch 的表现。
3. **AGA 的潜力**: 自适应几何感知 (AGA) 模式在极短的 20 Epoch 内就达到了 94.1% 的精度，达到并超越同期 `Sync Mode` ，展现了极高的计算效率。
