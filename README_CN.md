...
| **ARS2C (AGA)** | 94.11 | **352.27** | **Christoffel 动态 β** | `β₂∈[0.535,0.644]`, 当前最均衡 |
...
| 优化器 | 拟合 (Epoch) | 顿悟 (Epoch) | 收敛 (Epoch) | 最佳 Eval Acc | 状态 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **AdamW** | 113 | >600 | N/A | 15.65% | 严重泛化延迟，600 Epoch 未能实现顿悟。 |
| **Muon** | 22 | >347 | N/A | 36.83% | 纯几何优化在缺乏能量自适应时收敛极慢。 |
| **ARS2-Neo (Base)** | 11 | 239 | 290 | 99.53% | 能量-几何解耦实现相变。 |
| **ARS2-Neo (AGA)** | 12 | **77** | **116** | **99.60%** | **最优动力学**。AGA 将顿悟加速 3× 对比 Base。 |
| **ARS2C (AGA)** | **13** | **75** | **172** | 99.51% | **Christoffel 动态 β**。最快顿悟触发，Epoch 75。 |

**核心洞察**：ARS2-Neo (AGA) 将 Grokking 发生时间提前了 **7.8 倍以上**（对比 AdamW），有力证明了能量-几何解耦能避免模型在过拟合吸引盆中的无效游走，直接穿越高维峡谷抵达泛化解。
