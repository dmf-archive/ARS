...
为了验证优化器在泛化相变（Phase Transition）中的动力学特征，我们在模加法任务上对比了各优化器的表现。

| 优化器 | 拟合 (Epoch) | 顿悟 (Epoch) | 收敛 (Epoch) | 状态 |
| :--- | :--- | :--- | :--- | :--- |
| **AdamW** | ~140 | 228 | 556 | 标准 Grokking 曲线，存在显著延迟。 |
| **ARS** | **28** | **54** | 300 | **极速 Grokking**。泛化延迟几乎消失。 |
| **ARS2** | 17 | 100 | 290 | 稳健 Grokking。平坦度约束引导至更平坦区域。 |
| **Muon** | >156 | N/A | N/A | 在此特定任务配置下未收敛。 |

**核心洞察**：ARS 将 Grokking 发生时间提前了 **4 倍**，有力证明了能量-几何解耦能避免模型在过拟合吸引盆中的无效游走。
