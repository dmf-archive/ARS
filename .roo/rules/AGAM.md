...
| | α₁ 高 | α₁ 低 |
|:---|:---|:---|
| **α₂ 高** | 流形平滑 + Hessian 稳定 — SAM 路径最优 | 梯度旋转但二阶结构稳定（如过一阶鞍点）— 中间路径 |
| **α₂ 低** | 理论禁区（梯度稳定但 Hessian 突变需拟奇点结构） | 流形粗糙 + Hessian 不稳定 — GAM 路径必须 |
...
|:---|:---|
| 网格密度 `h(x)` | SAM 扰动半径 `ρ` |
| 监控函数 `ρ(x)` | Christoffel 范数 `‖C‖_F` |
| 等分布原理 | 曲率等分布触发阈值 |
| h-adaptive（细化/粗化） | GAM 路径开关 |
...
其中 `d` 为参数总数，`ε` 为训练精度（FP32: `1.19e-7`，FP16: `6.10e-4`，BF16: `7.81e-3`）。

**物理直觉**：参数空间的梯度范数分辨精度受限于浮点表示。当曲率变化幅度小于 `ε · √d` 时，任何二阶修正都无法与一阶近似的量化噪声区分——执行 GAM 路径的收益为零。

| 模型 | `d` | FP32 `τ_min` | BF16 `τ_min` | FP16 `τ_min` |
|:---|:---:|:---:|:---:|:---:|
| ResNet-18 (CIFAR-10) | 11M | `3.95e-4` | `25.91` | `2.02` |
| 3L Transformer (Wiki-2) | 5M | `2.67e-4` | `17.47` | `1.36` |
| Grokking (p=113) | 0.1M | `3.77e-5` | `2.47` | `0.19` |

Grokking 在 BF16 下实测 `c_magnitude` 在 `5~5000+` 范围——大部分步的曲率显著高于 `τ_min = 2.47`，MPS 不会错误抑制 GAM 触发。FP32 大模型的 `τ_min` 极小（`~10⁻⁴`），所有步实际由等分布条件控制。

**三层自适应**：

- 全局：`τ = max(τ_equidist, α · ε · √d)`，`α = 1.0` 默认
- 精度自适应：切换训练精度时阈值自动变化
- 层级别：`τ^(l) = ε · √(d_l)`，小参数量层对曲率更敏感
...
| 路径 | 曲率层 | 信噪比约束 | 实际 ρ | 条件 |
|:---|:---|:---|:---:|:---|
| SAM | 低 | ρ ≥ A | ρ_base | A 小（τ_min 量级），ρ_base 充足 |
| Middle | 中 | ρ ≥ A | ρ_base | A 适中，ρ_base 维持 |
| GAM | 高 | ρ ≥ √(10·A) | ρ_base | λ_max 极大 → A → 0，约束自动消失 |
...
- [6] ARS 系列设计文档: `ARS.md`, `ARS2.md`, `AR-GSAM.md`
- [7] IEST

---

- 状态：设计定稿，待实验验证
- 依赖：`ARS2.md` 中的 A-GSAM 同步机制，`AR-GSAM.md` 的失效诊断
