...
> **核心结论**: **AdaRMSuon** 与 **ARS (AdaRMSuon + SAM)** 优化器在模加法 (Modular Addition) 任务中，将 Grokking（顿悟）现象的发生时间相比 AdamW 基准提前了 **4 倍以上** (Epoch 228 -> Epoch 54)，有力证明了“能量-几何解耦”与“流形平坦度约束”在加速模型泛化相变中的关键作用。
...
- **动力学特征**: 在此特定任务配置下表现不佳。Train Loss 下降极慢（Epoch 156 时仍 > 4.0），由研究员手动取消。
- **原因推测**: Muon的几何约束导致有效学习率降低，尚待进一步调参。

### 2.3 AdaRMSuon

- **动力学特征**:
  - **极速拟合**: 训练集拟合速度比 AdamW 快 5 倍。
  - **提前顿悟**: 验证集准确率在 **Epoch 54** 就开始跃升，在拟合训练集的同时就开始了泛化，几乎不存在“死记硬背”到“理解规律”的延迟。
