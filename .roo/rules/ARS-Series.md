...
## Adaptive Geometric Awareness, A-GSAM

传统的静态周期 $k$ 无法适应动态变化的黎曼流形。A-GSAM 通过引入干涉因子实现“按需同步”，显著降低计算开销并提升收敛稳定性。**在未来的实验中，A-GSAM 将作为首选模式，取代传统的 Sync Mode。**
...
- `A-GSAM_beta` ($\beta$): 建议 0.9。控制几何统计量的平滑度。
- `A-GSAM_lambda` ($\lambda$): 控制同步触发的灵敏度，间接影响算力开销。 建议 0.5 (Wikitext-2) 或 2.0 (CIFAR-10)，取决于预算。
- `A-GSAM_gamma` ($\gamma$): 建议 2.0。控制自适应强度律的非线性程度。
...
| **ARS2C-AR-GSAM** | 13 | 98 | **N/A** | **98.86%** |
| **ARS2DC-AR-GSAM** | 13 | 79 | **204** | **99.16%** |

### AR-GSAM 变体诊断分析

> 状态：AR-GSAM（动态 ρ）在 Grokking 任务上表现不如预期。结合组合收敛定律，问题定位在 `cos_sim` 的**系统性偏低**导致的动态 ρ 过膨胀。

#### 关键诊断数据（ARS2C-AR-GSAM Epoch 267）
...
**真正的瓶颈在于：** AR-GSAM 的 `cos_sim` 信号在 Grokking 的高精度微调阶段失去鉴别力 —— 当模型接近最优解时，梯度小而曲率变化大，两个方向虽然都蕴含几何信息但严重失配，导致 ρ 调制退化为随机游走。

**ARS2DC-AR-GSAM（204ep 99.16%，2x 慢于 ARS2D A-GSAM）：**
...
2. 在高精度阶段（>98%）收敛极慢：95%→98% 用 50ep（vs ARS2D A-GSAM 的 19ep）
3. 但双正交化的**更平坦诱导**最终帮助模型在 204ep 跨过 99% 阈值（vs ARS2C-AR-GSAM 从未达到）

#### 待解决问题

- AR-GSAM 的 `cos_sim` 信号需要重新设计：可能用行列对齐矩阵的均值替代全局标量，或引入 `alignment_mean` 作为 `cos_sim` 的替代信号
- 双正交化场景下，`c_ortho` 也应做双正交化以匹配 `s_ortho` 的几何深度
- New AR-GSAM 的 `f_a = 1 + (1 - cos_sim)` 线性响应在 `cos_sim` 接近 0 时过于激进，可能需引入 sigmoid 门控
