...
| ARS2C | ARS2 + Christoffel | +动态β |
| ARS2C-AGA | ARS2C + AGA | +动态β + 自适应同步 |
| ARS2C-SAGA | ARS2C + SAGA | +动态β + 动态ρ（+自适应同步） |
...
| **ARS2C (AGA, Unlock)** | 94.11 | **352.27** | ~630s |
...
| **ARS2C (AGA)** | **13** | **75** | **172** | 99.51% |

## ARS2-Neo：重构和整合后的参考版本

ARS2-Neo 在统一的代码中实现了 AdaRMSuon 的几何优化与 SAM 的平坦度约束，通过参数配置灵活切换模式，以取代实验性的独立 `AdaRMSuon` 和 `ARS`。随着 ARS2-Neo 的成熟，我们将逐步移除旧的实验性优化器代码，以简化实验空间。

## ARS2C：Christoffel-Aware Dynamic Beta Optimization

ARS2C 是 ARS2 家族的最新成员，将固定的动量衰减率 β₁, β₂ 替换为从 Fisher 信息流形推导的 Christoffel 感知动态 β。

### 关键实验结果

| 指标 | ARS2-Neo (AGA) | ARS2C (AGA, Unlock) | 改善 |
| :--- | :--- | :--- | :--- |
| Best PPL | 93.23 | 94.11 | 持平 |
| Final PPL | 414.83 | **352.27** | **−15.1%** |
| β₂ 动态范围 | 固定 0.95 | [0.535, 0.644] | 真正动态 |
| alignment 范围 | N/A | [0.071, 0.288] | 信号活跃 |

ARS2C 在 AGA 模式下以 ~15% 的计算开销换取了 15% 的最终 PPL 改善，追平 ARS2-Neo-Sync ，且 β₂ 展现出有意义的动态范围——从 0.535 到 0.644 的连续调节，验证了 Christoffel 感知遗忘的核心假设。
