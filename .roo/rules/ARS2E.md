...
ARS2E 在 [`ARS2`](.roo/rules/ARS2.md:1) 的能量-几何解耦之上，用离散 EFE 替代 Christoffel 行列余弦归一化，构造**保幅度的离散联络**以驱动动量衰减 β₁, β₂ 的动态调度。
...
| IEST 项 | 优化器对应 | 物理含义 |
|---------|-----------|---------|
| 度量 γ_ab | Fisher 信息 G_ij，对角近似 v̂_t | 参数空间局部几何 |
| 能动量 Σ_ab | 梯度流 ∇L | 更新动力 |
| 离散协变导数 Δ_𝒢 | 参数流形上的有限差分 | 几何修正 |
| 曲率 H_ab = Δ_𝒢 Σ_ab + ½ ℛ_𝒢[γ] Σ_ab | 联络调制梯度 | β 调度的几何来源 |
| 普朗克压强 P_L · Σ_ab γ_ab | MPS 截断项 ε·√d · g · v̂ | AGAM 触发阈值下界 |
| 右端项 κ·T_P·Σ_ab | 学习率 × 梯度 η·g | 参数更新步 |
...
| 属性 | δ_g (ARS2C) | Δv̂/v̂ (ARS2E) |
|------|------------|---------------|
| 信息来源 | 梯度空间 H·ĝ | 度量空间 Fisher 变化率 |
| 幅度保真 | 行列归一化后丢失 | 天然保留 |
| 计算开销 | 需 SAM 双前向才有 δ_g | 每步可算（v̂ 已存在） |
| 噪声水平 | 随批量采样波动大 | EMA 平滑后稳定 |
| 曲率符号 | H·ĝ 的方向 | Γ 的正负自然编码扩张/收缩 |
...
| alignment_preserved | 几何条件 | β 行为 |
|-------------------|---------|--------|
| → 1 (高) | 度量稳定，曲率方向与更新方向对齐 | β → β_max（强滤波，保留历史） |
| → 0 (低) | 度量剧变，曲率方向与更新方向失配 | β → β_min（快速遗忘，适应突变） |
| 介于之间 | 中等曲率变化 | 连续插值 |

### 4.2 与 ARS2C 的关键差异

| 属性 | ARS2C (行列余弦) | ARS2E (度量差) |
|------|-----------------|----------------|
| β 驱动信号 | row_alignment (幅度死亡) | ‖H_ab‖_F × cos_dir (幅度保真) |
| 低曲率行为 | alignment 仍为中间值 (0.5) | 自动衰减到近 0 |
| 高曲率行为 | alignment 仍为中间值 (0.5) | 激活到近 1 |
| 动态范围 | < 2% | > 100× |
| 理论来源 | Christoffel 类比 | IEST |
...
P_L · Σ_ab γ_ab  →  MPS 截断  →  AGAM 阈值 (AGAM.md)
```
...
| 层 | 负责 | 机制 | 文档 |
|----|------|------|------|
| ARS 侧 | NGD 测地线，β 调度 | 离散 EFE → 联络 → β 调制 | ARS2E.md (本文) |
| SAM/GAM 侧 | MDL 拉格朗日，扰动深度 | 等分布 + MPS → AGAM 触发 | AGAM.md |
...
- AGAM 设计文档：[`AGAM.md`](.roo/rules/AGAM.md:1)
- AR-GSAM 设计文档：[`AR-GSAM.md`](.roo/rules/AR-GSAM.md:1)
- ARS2 设计文档：[`ARS2.md`](.roo/rules/ARS2.md:1)
