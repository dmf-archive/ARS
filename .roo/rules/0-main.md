...
- **REQ-501**: **GPU 张量累积禁令**: 严禁在训练循环中使用 `.detach()` 累积 GPU 张量到列表或其他集合中。这种做法会导致 VRAM 占用呈线性增长，最终引发内存爆炸。所有跨 step 的统计量计算必须使用流式累加（标量累加和/计数器）而非事后聚合。
- **REQ-502**: **流式计算优先**: 对于需要跨 step 聚合的统计量（如平均值、熵值等），必须优先使用流式计算模式：在每个 step 中计算当前批次贡献并累加到标量变量中，在 epoch 结束时用累加值计算最终结果。
- **REQ-503**: **实时可观测性义务**: 所有训练实验必须提供实时可观测性，严禁仅在训练结束后才生成报告。Summary 文件必须在每个 epoch 结束时更新，确保用户能够实时监控训练进度和性能指标。
