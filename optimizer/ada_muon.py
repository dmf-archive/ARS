...
class AdaMuon(Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            is_muon_group = group.get('is_rmsuon_group', False) # Reuse flag for simplicity

            if is_muon_group:
                self._ada_muon_step(group)
            else:
                self._adamw_step(group)

        return loss

    def _ada_muon_step(self, group: dict):
