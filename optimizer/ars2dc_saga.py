...
class ARS2DCSAGA(Optimizer):
    ...
    def _decide_sync(self) -> bool:
        ...
        if is_aga:
            ...
            is_sync = is_drift or (global_step == 1) or (k > 1 and steps_since_sync >= k)
            ...
            return global_step % k == 1 if k > 1 else True
    ...
    def step(self, closure: Callable[[], torch.Tensor] | None = None) -> torch.Tensor | None:
        ...
        is_sync_step = self._decide_sync()
        ...
        for group in self.param_groups:
            if group.get('is_rmsuon_group', False):
                self._update_ars(group, global_step)
            else:
                self._update_adamw(group, global_step)
    ...
    def _update_adamw(self, group: dict, global_step: int):
        beta1, beta2 = group['betas']
        lr = group['lr']
        eps = group['eps']
        weight_decay = group['weight_decay']
    ...
    def _apply_ars2_kernel(self, p, beta1, beta2, lr, eps, weight_decay, ns_steps):
        ...
        if is_christoffel and alignment_matrix is not None and p.ndim >= 2:
    ...
    def diagnostics(self) -> dict:
        ...
        if rhos:
            ...
            rho_mean = d['rho_mean']
            d['rho_std'] = (sum((r - rho_mean) ** 2 for r in rhos) / len(rhos)) ** 0.5
