...
class F3EL(Optimizer):
    def __init__(self,
        ...
        super(F3EL, self).__init__(params, defaults)
        ...
        for group in self.param_groups:
            for p in group['params']:
                if p.grad is not None:
                    if p.grad.is_sparse:
                        raise RuntimeError('F3EL does not support sparse gradients.')
            ...
            for group in self.param_groups:
                for p in group['params']:
                    ...
                    if meta_grad is None:
                        ...
                        if group['loss_scaling'] and main_loss is not None:
                            effective_grad = first_grad - main_loss.detach() * meta_grad
                        else:
